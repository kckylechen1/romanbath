...
export const LeftSidebar: React.FC<LeftSidebarProps> = ({
  ...
  return (
    <aside
      className={`${leftSidebarOpen ? 'w-80' : 'w-20'} hidden md:flex flex-col glass-panel border-r border-bath-700/15 transition-all duration-500 ease-[cubic-bezier(0.25,1,0.5,1)] z-30 bath-reveal bath-reveal-delay-1`}
    >
      <div className="flex items-center justify-between p-6 border-b border-bath-700/15">
        <div
        ...
        >
          ...
          <span className="font-display font-bold text-lg tracking-tight text-bath-100">
            {t('app.title')} <span className="text-bath-500 font-sans text-sm">V2</span>
          </span>
        </div>
        <button
          onClick={() => setLeftSidebarOpen(!leftSidebarOpen)}
          className="text-bath-500 hover:text-bath-100 transition-colors"
