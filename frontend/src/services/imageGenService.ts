...
export const isPhotoRequest = (text: string): boolean => {
  ...
  return new RegExp(`${verbs.source}.{0,12}${nouns.source}`, "i").test(normalized)
    || new RegExp(`${nouns.source}.{0,12}(?:发|傳|传|来|给|給|看看|see|show|send)`, "i")
      .test(normalized);
};
...
export const buildCharacterPhotoPrompt = (
  ...
  const baseIdentity =
    identity ||
    [
      `The character is ${character.name}.`,
