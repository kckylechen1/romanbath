...
// ── xAI impl ──────────────────────────────────────────────────────────
//
// xAI supports OAuth2 (PKCE browser flow, device code flow) and API-key
// bearer tokens. The login/refresh flows delegate to the `xai_oauth`
// module. `paste-redirect` follows the same pending-login pattern as
